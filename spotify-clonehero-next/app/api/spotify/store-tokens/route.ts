...
export async function POST() {
  try {
    ...
    const expiresInSec = (session as any).provider_token_expires_in as number | undefined

    if (!accessToken) {
      return NextResponse.json({ ok: false, error: 'No provider token on session' }, { status: 200 })
    }
