 ...
 *
 *  1. Map midi pitches to drum staff keys (via interpretDrumNote, which
 *     applies pro-drums tom markers and disco flip). Drum notation puts
 *     everything on one voice with stems down, so kicks, snares, toms and
 *     cymbals that share a tick become a single chord.
...
export interface Note {
  ...
  tick: number;
  /**
   * Audio time of the hit, from the note's *original* chart tick (before any
   * grid regularization) so the playhead reaches the notehead exactly when it
   * sounds. 0 for rests (rests have no playhead entry).
   */
  ms: number;
...
/** A note plus the original onset it notates (null for rests). */
...
function resolveCluster(cluster: Onset[]): Onset {
  ...
  occurrences.forEach(({tick, key, id, accent, ghost}) => {
  ...
  const graceByTick = new Map<number, Map<string, string | null>>();

  occurrences.forEach(({tick, key, id}) => {
    if (tick < (lastTickByKey.get(key) as number)) {
      const keys = graceByTick.get(tick) ?? new Map<string, string | null>();
  ...
  const graceEntries = [...graceByTick.entries()].sort((a, b) => a[0] - b[0]);
  const graceNotes = graceEntries.map(([, keys]) => sortKeys([...keys.keys()]));
  const graceNoteIds = graceEntries.map(([, keys], chordIndex) =>
    graceNotes[chordIndex].map(key => keys.get(key) ?? null),
  );
  ...
  return {
    ...
    graceNoteIds: graceNotes.length > 0 ? graceNoteIds : undefined,
  };
}

function makeMeter(numerator: number, denominator: number, ppq: number): Meter {
...
function complexityOf(events: PositionedNote[]): number {
  let filler = 0;
  let fine = 0;
  ...
  events.forEach((event, index) => {
    ...
    if (
      event.note.isRest &&
      events.slice(0, index).some(e => !e.note.isRest) &&
      events.slice(index + 1).some(e => !e.note.isRest)
    ) {
      filler += 1;
    }

    fine += halvingsPast16th(event.note.duration);
    values.add(`${event.note.duration}.${event.note.dots}`);
  });
...
function makeRest(tick: number, duration: string, dots: number): Note {
  return {
    notes: [REST_KEY],
    noteIds: [null],
    duration,
...
function restFill(startTick: number, fraction: number): Candidate {
  ...
  return {
    events: [
      {
        note: makeRest(Math.round(startTick), named.duration, named.dots),
        onset: null,
      },
    ],
    tuplets: [],
    ...
    chunkSpan(boundary, next - boundary, slotFraction, true).forEach(
      (size, chunkIndex) => {
        ...
        if (named) {
          ...
          const keys = isRest ? [REST_KEY] : sortKeys(onset.keys);
          ...
          events.push({
            note: {
              ...
              noteIds: isRest
                ? [null]
                : keys.map(key => onset.idOf[key] ?? null),
    ...
    return noteEventGroups
      .map((group): Onset | null => {
        ...
        group.forEach(note => {
          ...
          const key = interpreted.isDoubleKick
            ? DOUBLE_KICK_KEY
            : mapping[interpreted.instrument];

          keys.push(key);
          idOf[key] = fillNoteIdFromRaw(tick, note)?.id ?? null;

          if (interpreted.dynamic === 'accent') {
            accents.push(key);
          }

          if (interpreted.dynamic === 'ghost') {
            ghosts.push(key);
          }
        });
    ...
    while (beatIndex < meter.beatsPerMeasure) {
      if (beatOnsets[beatIndex].length === 0) {
        ...
        chunkSpan(beatIndex, run, meter.beatFraction, meter.isCompound).forEach(
          size => {
            ...
            if (named) {
              notes.push(
                makeRest(
                  Math.round(measure.startTick + slot * meter.beatTicks),
                  named.duration,
                  named.dots,
                ),
              );
        ...
        candidate.events.forEach(event => {
          // Audio time comes from the original onset (not the notated grid
          // slot) so the playhead reaches the notehead exactly when the hit
          // sounds, even when the engine regularized its written position.
          if (event.onset) {
            event.note.ms = tickToMs(this.chart, event.onset.tick);
          }
