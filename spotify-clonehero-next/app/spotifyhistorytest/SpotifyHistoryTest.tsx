...
async function getData() {
  ...
  //   .innerJoin('spotify_track_chart_matches as sc', 'spot.id', 'sc.spotify_id')
  //   .innerJoin('chorus_charts as cc', 'sc.chart_md5', 'cc.md5')
  //   .leftJoin('local_charts as lc', join =>
  //     join
  //       .onRef('lc.artist_normalized', '=', 'cc.artist_normalized')
  //       .onRef('lc.song_normalized', '=', 'cc.name_normalized')
  //       .onRef('lc.charter_normalized', '=', 'cc.charter_normalized'),
  //   )
  //   .select([
  //     'spot.id as spotify_id',
  //     'spot.name as spotify_name',
  ...
  //   .selectAll('cc')
  //   .select(
  //     sql<number>`CASE WHEN lc.artist_normalized IS NOT NULL THEN 1 ELSE 0 END`.as(
  //       'is_installed',
  //     ),
  //   )
  //   // .where('lc.artist_normalized', 'is not', null)
  //   // .limit(10)
  //   .execute();

  // Subquery: spot → sc → cc to get the normalized keys for each spotify track
  const base = db
    .selectFrom('spotify_tracks as spot')
    .innerJoin('spotify_track_chart_matches as sc', 'spot.id', 'sc.spotify_id')
    .innerJoin('chorus_charts as cc', 'sc.chart_md5', 'cc.md5')
  ...
  return rows.map(r => {
    r.matching_charts = JSON.parse(r.matching_charts);
    return r;
  });
}
