...
export default function EditorMCPTools() {
  ...
  useEffect(() => {
    ...
    register({
      name: 'editor_state',
      description: 'Get current editor state: selection, tool, position, chart info.',
        ...
        if (s.chartDoc && s.selectedNoteIds.size > 0) {
          ...
          if (track) {
            selectedNotes = getDrumNotes(track)
              ...
              .map(n => ({id: noteId(n), tick: n.tick, type: n.type, flags: {cymbal: !!n.flags.cymbal, accent: !!n.flags.accent, ghost: !!n.flags.ghost}}));
          }
        }
        return {content: [{type: 'text', text: JSON.stringify({
          activeTool: s.activeTool, cursorTick: s.cursorTick, gridDivision: s.gridDivision,
    ...
    register({
      ...
      inputSchema: {type: 'object', properties: {timeMs: {type: 'number'}, tick: {type: 'number'}}},
      execute: async (args) => {
        const am = audioManagerRef.current;
        const s = stateRef.current;
        ...
        await am.pause();
        return {content: [{type: 'text', text: `Seeked to ${seekMs.toFixed(0)}ms`}]};
      },
    });
    ...
    register({
      ...
      inputSchema: {type: 'object', properties: {startTick: {type: 'number'}, endTick: {type: 'number'}, limit: {type: 'number'}}},
      execute: async (args) => {
        const s = stateRef.current;
        ...
        const notes = getDrumNotes(track).filter(n => n.tick >= startTick && n.tick <= endTick).slice(0, limit)
          .map(n => ({id: noteId(n), tick: n.tick, type: n.type, lane: typeToLane(n.type), flags: {cymbal: !!n.flags.cymbal, accent: !!n.flags.accent, ghost: !!n.flags.ghost}}));
    ...
    register({
      ...
      inputSchema: {type: 'object', properties: {tick: {type: 'number'}, type: {type: 'string'}, addToSelection: {type: 'boolean'}}, required: ['tick', 'type']},
      execute: async (args) => {
    ...
    register({
      ...
      execute: async (args) => {
        ...
        if (!['cymbal', 'accent', 'ghost'].includes(flag)) return {content: [{type: 'text', text: 'Invalid flag'}]};
        const s = stateRef.current;
        if (s.selectedNoteIds.size === 0) return {content: [{type: 'text', text: 'No notes selected'}]};
        executeCommandRef.current(new ToggleFlagCommand(Array.from(s.selectedNoteIds), flag));
        return {content: [{type: 'text', text: `Toggled ${flag} on ${s.selectedNoteIds.size} note(s)`}]};
      },
    });
    ...
    register({
      ...
      inputSchema: {type: 'object', properties: {tick: {type: 'number'}, type: {type: 'string'}, cymbal: {type: 'boolean'}}, required: ['tick', 'type']},
      execute: async (args) => {
        const type = args.type as DrumNoteType;
        const tick = args.tick as number;
        const cymbalDefault = type === 'yellowDrum' || type === 'blueDrum' || type === 'greenDrum';
        const cymbal = (args.cymbal as boolean) ?? cymbalDefault;
        executeCommandRef.current(new AddNoteCommand({tick, type, length: 0, flags: {cymbal}}));
        return {content: [{type: 'text', text: `Added ${type} at tick ${tick}`}]};
      },
    });
    ...
    register({
      ...
      execute: async () => {
        ...
        return {content: [{type: 'text', text: `Deleted ${s.selectedNoteIds.size} note(s)`}]};
      },
    });
    ...
    register({
      ...
      description: 'Switch active tool: cursor, place, erase, bpm, timesig, section',
    ...
    register({
      ...
      inputSchema: {type: 'object', properties: {}},
      execute: async () => {
        if (!canUndoRef.current) return {content: [{type: 'text', text: 'Nothing to undo'}]};
        undoRef.current();
    ...
    register({
      ...
      inputSchema: {type: 'object', properties: {}},
      execute: async () => {
        if (!canRedoRef.current) return {content: [{type: 'text', text: 'Nothing to redo'}]};
        redoRef.current();
    ...
    return () => {
      ...
      if (navigator.modelContext && typeof (navigator.modelContext as any).unregisterTool === 'function') {
        for (const name of toolNames) {
          try { (navigator.modelContext as any).unregisterTool(name); } catch { /* ignore */ }
        }
      }
    };
