 ...
 * `EditorTool`(s) that handle it for `useHighwayMouseInteraction`.
 *
 * `'cursor'` is the one mode with more than one tool: `selectMoveTool` and
 * `boxSelectTool`, chosen at pointer-down by whether a selectable entity is
 * under the cursor, and at pointer-move/up by whether a note drag is in
 * flight (a gesture started by `selectMoveTool` must finish there even if
 * the cursor drifts off every entity mid-drag).
 *
 ...
 * decided by what gesture is already in flight: a note drag started by
 * `selectMoveTool` must keep routing there even once the pointer has drifted
 * off every entity; otherwise it's `boxSelectTool`'s marquee.
 */
export function resolveCursorContinuation(ctx: ToolContext): EditorTool {
  if (ctx.drag.isDragging) return selectMoveTool;
  return boxSelectTool;
}
