 ...
 */
import { countNotesByVoice, DrumVoice, isCymbal } from '../drumLaneMap.js';
 ...
 */
export function computeWindowFeatures(window, config, resolution, rollingStats) {
    const notes = window.notes;
    const windowDurationBeats = ticksToBeats(window.endTick - window.startTick, resolution);
    ...
    const kickRatio = totalNotes > 0 ? voiceCounts[DrumVoice.KICK] / totalNotes : 0;
    ...
    const densityZ = rollingStats ?
        (rollingStats.densityStd > 0 ? (noteDensity - rollingStats.densityMean) / rollingStats.densityStd : 0) : 0;
    const tomRatioJump = rollingStats ?
        (rollingStats.tomRatioMean > 0 ? tomRatio / rollingStats.tomRatioMean : 1) : 1;
    const hatDropout = rollingStats ?
        Math.max(0, 1 - (rollingStats.hatRatioMean > 0 ? hatRatio / rollingStats.hatRatioMean : 1)) : 0;
    const kickDrop = rollingStats ?
        Math.max(0, rollingStats.kickRatioMean - kickRatio) : 0;
 ...
 */
function calculateNgramNovelty(notes, resolution) {
    if (notes.length === 0)
        return 0;
 ...
 */
function detectSamePadBurst(notes, burstThresholdMs) {
    if (notes.length < 3)
        return false;
    // Group notes by type
 ...
 */
function detectCrashResolve(window, resolution) {
    const notes = window.notes;
    if (notes.length === 0)
        return false;
    ...
    const lateCrashes = notes.filter(note => note.tick >= lastQuarter && isCymbal(note.type));
    if (lateCrashes.length === 0)
        return false;
    // Check if the next downbeat after the window would be a likely resolution point
    const nextDownbeat = Math.ceil(window.endTick / (resolution * 4)) * (resolution * 4);
 ...
 */
export function extractFeaturesFromWindows(windows, config, resolution) {
    if (windows.length === 0)
        return [];
...
function updateRollingStatistics(windows, config) {
    ...
    for (let i = 0; i < windows.length; i++) {
        ...
        const rollingStats = {
            ...
            hatRatioMean: 1 - hatDropouts.reduce((a, b) => a + b, 0) / hatDropouts.length,
 ...
 */
export function createAnalysisWindows(notes, startTick, endTick, windowBeats, strideBeats, resolution, tempos // TempoEvent[] but avoiding circular import
) {
    const windows = [];
...
//# sourceMappingURL=windowStats.js.map