 ...
 */
export function mahalanobisDistance(point, mean, covarianceInverse) {
 ...
 */
export function regularizeCovariance(cov, regularization = 1e-6) {
 ...
 */
export function rollingStdDev(values, windowSize) {
    if (windowSize <= 0 || windowSize > values.length) {
        throw new Error('Invalid window size');
    }
    const result = [];
