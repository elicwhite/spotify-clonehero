...
import { extractFills, defaultConfig } from './dist/index.js';
...
async function runExample() {
  ...
  console.log(`📈 Chart contains ${chart.trackData[0].noteEventGroups[0].length} drum notes\n`);
  ...
  if (fills.length === 0) {
    ...
    console.log(`✅ Found ${fills.length} drum fill${fills.length > 1 ? 's' : ''}:\n`);
    
    fills.forEach((fill, index) => {
      const duration = (fill.endMs - fill.startMs) / 1000;
      console.log(`Fill #${index + 1}:`);
      console.log(`  📍 Time: ${(fill.startMs / 1000).toFixed(2)}s - ${(fill.endMs / 1000).toFixed(2)}s (${duration.toFixed(2)}s)`);
      ...
      console.log(`  🎯 Features: ${JSON.stringify({
  ...
  if (sensitiveFills.length > fills.length) {
    ...
    sensitiveFills.slice(fills.length).forEach((fill, index) => {
      const duration = (fill.endMs - fill.startMs) / 1000;
      console.log(`  Additional Fill #${index + 1}: ${(fill.startMs / 1000).toFixed(2)}s - ${(fill.endMs / 1000).toFixed(2)}s`);
    });
    console.log('');
  }
