...
async function scanCloneHeroSongs() {
  ...
  try {
    ...
    for (const dir of directories) {
      ...
      try {
        ...
        if (hasDirectSong) {
          ...
          const subDirectories = subEntries.filter(entry => entry.isDirectory());
    ...
    if (error.code === 'ENOENT') {
      ...
      console.error('Please make sure Clone Hero is installed and the Songs directory exists.');
    } else {
      console.error('Error scanning Clone Hero Songs directory:', error.message);
    }
  }
}
