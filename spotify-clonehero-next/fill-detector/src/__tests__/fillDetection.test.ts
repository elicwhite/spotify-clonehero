import { extractFills, defaultConfig } from '../index.js';
import { validateConfig } from '../config.js';
import { NoteEvent, ParsedChart, Config } from '../types.js';
...
function createTomFill(startTick: number, noteCount = 16, resolution = 192): NoteEvent[] {
  const notes: NoteEvent[] = [];
  ...
  describe('extractFills', () => {
    ...
    it('should detect multiple fills in a song', () => {
      ...
      const notes = [
        ...
        ...createBasicDrumPattern(4, resolution).map(n => ({ ...n, tick: n.tick + 5 * resolution * 4 })),
