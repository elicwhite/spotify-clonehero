...
describe('Tempo Utilities', () => {
  ...
  describe('buildTempoMap', () => {
    it('should build tempo map from single tempo', () => {
      ...
      expect(tempoMap[0].bpm).toBe(120);
    ...
    it('should build tempo map from multiple tempos', () => {
      const tempos: TempoEvent[] = [
        { tick: 0, bpm: 120, msTime: 0 },
        { tick: 1920, bpm: 140, msTime: 5000 }, // 10 beats later, faster tempo
        { tick: 3840, bpm: 100, msTime: 11000 }, // 10 beats later, slower tempo
      ];
      ...
      expect(tempoMap[0].bpm).toBe(120);
      expect(tempoMap[1].bpm).toBe(140);
      expect(tempoMap[2].bpm).toBe(100);
    });
  ...
  describe('tickToMs', () => {
    ...
    it('should handle tempo changes correctly', () => {
      const tempos: TempoEvent[] = [
        { tick: 0, bpm: 120, msTime: 0 },      // 0-1920: 120 BPM
        { tick: 1920, bpm: 60, msTime: 5000 }, // 1920+: 60 BPM (slower)
      ];
  ...
  describe('edge cases', () => {
    ...
    it('should handle requests for ticks before first tempo event', () => {
      const tempos: TempoEvent[] = [
        { tick: 100, bpm: 120, msTime: 1000 } // First tempo not at tick 0
      ];
