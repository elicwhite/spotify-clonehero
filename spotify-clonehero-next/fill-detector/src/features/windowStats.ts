...
import { countNotesByVoice, DrumVoice, isTom, isHat, isKick, isCymbal } from '../drumLaneMap.js';
import { ticksToBeats, isDownbeat } from '../quantize.js';
 ...
 */
function detectSamePadBurst(notes: NoteEvent[], burstThresholdMs: number): boolean {
 ...
 */
function detectCrashResolve(window: AnalysisWindow, resolution: number): boolean {
  const notes = window.notes;
  ...
  const lateCrashes = notes.filter(note => 
    note.tick >= lastQuarter && isCymbal(note.type)
  );
 ...
 */
function updateRollingStatistics(windows: AnalysisWindow[], config: Config): void {
  ...
  for (let i = 0; i < windows.length; i++) {
    ...
    windows[i].features = computeWindowFeatures(
      windows[i], 
      config, 
      0, // resolution not needed for recomputation
      rollingStats
    );
  }
}
...
export function createAnalysisWindows(
  ...
  for (let windowStart = startTick; windowStart + windowTicks <= endTick; windowStart += strideTicks) {
    ...
    const windowNotes = notes.filter(note => 
      note.tick >= windowStart && note.tick < windowEnd
    );
