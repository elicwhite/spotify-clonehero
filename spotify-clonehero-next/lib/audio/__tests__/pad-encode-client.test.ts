/**
 * The worker boundary for pad + WAV-encode (`pad-encode-client.ts`): request
 ...
 * `pad-encode-worker.ts` (same convention as
...
import type {PadEncodeRequest, PadEncodeWorkerMessage} from '../pad-encode';
import {padAndEncodeTracks} from '../pad-encode-client';
import {isAbortError} from '@/lib/workers/abortable-worker';

class FakeWorker {
  onmessage: ((e: {data: PadEncodeWorkerMessage}) => void) | null = null;
  onerror: ((e: {message?: string}) => void) | null = null;
  posted: Array<{request: PadEncodeRequest; transfer: unknown}> = [];
  terminated = false;

  postMessage(request: PadEncodeRequest, transfer?: unknown) {
    this.posted.push({request, transfer});
  }
  terminate() {
    this.terminated = true;
  }
  emit(message: PadEncodeWorkerMessage) {
    this.onmessage?.({data: message});
  }
}

const PARAMS = {padSamples: 8, sampleRate: 44100, channels: 2};
...
describe('padAndEncodeTracks', () => {
  it('posts the tracks with no transfer list, so the caller keeps its PCM', async () => {
    ...
    const promise = padAndEncodeTracks([{name: 'song', pcm}], {
      ...PARAMS,
      createWorker,
    });

    const {request, transfer} = worker().posted[0];
    expect(request.type).toBe('pad-encode');
  ...
  it('forwards progress events and resolves with the worker result', async () => {
    const seen: number[] = [];
    const {createWorker, worker} = spawn();
    const promise = padAndEncodeTracks(
    ...
    const encoded = [
      {
        name: 'song',
        paddedPcm: new Float32Array(2),
        wav: new Uint8Array(44),
      },
    ];
    worker().emit({type: 'result', tracks: encoded});

    await expect(promise).resolves.toEqual(encoded);
  ...
  it('rejects an already-aborted signal without spawning a worker', async () => {
    ...
    await expect(
      padAndEncodeTracks([{name: 'song', pcm: new Float32Array(4)}], {
        ...PARAMS,
        createWorker,
  ...
  it('runs inline, with the same result, where there is no Worker', async () => {
    ...
    const encoded = await padAndEncodeTracks([{name: 'song', pcm}], {
      padSamples: 1,
      sampleRate: 44100,
      channels: 2,
      createWorker: null,
    ...
    expect(encoded[0].paddedPcm.length).toBe(4);
    expect(encoded[0].wav.length).toBe(44 + 4 * 2);
  });
});
