...
describe('isCommandAllowed', () => {
  it('DRUM_EDIT allows every kind (notes, sections, lyrics, tempo, timesig)', () => {
    ...
    expect(isCommandAllowed(tempoMoveCmd(), DRUM_EDIT_CAPABILITIES)).toBe(
      true,
    );
  ...
  it('ADD_LYRICS allows lyric edits but rejects notes/tempo/timesig', () => {
    ...
    expect(isCommandAllowed(sectionCmd(), ADD_LYRICS_CAPABILITIES)).toBe(
      false,
    );
  ...
  it('PREVIEW rejects every command (read-only)', () => {
    ...
    expect(isCommandAllowed(tempoMoveCmd(), PREVIEW_CAPABILITIES)).toBe(
      false,
    );
  });
  ...
  it('a tempo marker move that KEEP-MS-remaps note ticks is gated by its ' +
  ...
  it('BatchCommand gates as the union of its members: one disallowed ' +
