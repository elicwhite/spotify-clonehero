...
import {EventType, Instrument, NotesData, TrackEvent} from 'scan-chart-web';
...
export class ChartParser {
  ...
  private getResolution() {
    ...
    if (!resolution) {
      this.notesData.chartIssues.push('noResolution');
    }
    return resolution;
  }
  ...
  private getTimeSignatures() {
    ...
    for (const line of syncTrack) {
      ...
      timeSignatures.push({
        tick,
        value: numerator / Math.pow(2, denominatorExp),
      });
    }
  ...
  public parse(): ChartParser {
    if (
      !this.resolution ||
      !this.tempoMap.length ||
      !this.timeSignatures.length
    ) {
      return this; //{notesData: this.notesData, notesMetadata: this.getMetadata()};
    }
  ...
  private setMissingExperts() {
    const missingExperts = _.chain(
      ...
      // @ts-ignore This doesn't error in scan-chart, not sure why it errors here
 ...
 */
export function parseChart(buffer: ArrayBuffer) {
