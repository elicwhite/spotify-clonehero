...
export class TrackParser {
  ...
  constructor(
    ...
    this.notes = trackEvents.filter(
      event => !nonNoteEvents.includes(event.type),
    );
  ...
  public parseTrack() {
    ...
    if (this.groupedNotes.length > 0) {
      ...
      // Add note count
      this.notesData.noteCounts.push({
        instrument: this.instrument,
        difficulty: this.difficulty,
        count: this.groupedNotes.length,
      });
    } else {
      this.notesData.instruments = this.notesData.instruments.filter(
        i => i !== this.instrument,
      );
    ...
    if (this.instrument !== 'drums') {
      for (let i = 1; i < this.groupedNotes.length; i++) {
        ...
        if (distance > 0 && distance <= 15) {
          if (
            (this.typeCount(note, [EventType.open]) > 0 &&
              this.typeCount(previousNote, [EventType.open]) === 0) ||
            (this.typeCount(previousNote, [EventType.open]) > 0 &&
              this.typeCount(note, [EventType.open]) === 0)
          ) {
    ...
    if (
      this.groupedNotes[0]?.time !== undefined &&
      this.groupedNotes[0].time < LEADING_SILENCE_THRESHOLD_MS
    ) {
    ...
    if (this.noteIssues.length) {
      this.noteIssues = _.chain(this.noteIssues)
        ...
        .sortBy(n => n.time)
        .value();
      this.notesData.noteIssues.push({
        instrument: this.instrument,
        difficulty: this.difficulty,
        noteIssues: this.noteIssues,
      });
    }
    if (this.trackIssues.length) {
      this.notesData.trackIssues.push({
        instrument: this.instrument,
        difficulty: this.difficulty,
  ...
  private parseDrumTrack() {
    ...
    const tomOrCymbalMarkerIds = [
      EventType.blueTomOrCymbalMarker,
    ...
    for (const event of this.trackEvents) {
      ...
      if (tomOrCymbalMarkerIds.includes(event.type)) {
        trackHasCymbalMarkers = true;
      }
      if (event.type === EventType.green) {
        trackHasFiveLaneGreenNote = true;
      }
      if (drumNoteIds.includes(event.type) && event.length > 0) {
        trackHasSustains = true;
      }
      if (event.type === EventType.kick2x) {
        this.notesData.has2xKick = true;
      }
      if (
        event.type === EventType.rollLaneSingle ||
        event.type === EventType.rollLaneDouble
      ) {
    ...
    if (
      trackHasCymbalMarkers &&
      (trackHasFiveLaneGreenNote || trackHasSustains)
    ) {
      this.addTrackIssue('has4And5LaneFeatures');
    }

    const nonKickDrumNoteIds = [
      EventType.green,
      EventType.red,
      EventType.yellow,
      EventType.blue,
    ...
    for (const note of this.groupedNotes) {
      ...
      if (this.difficulty === 'easy') {
        if (
          this.typeCount(note, nonKickDrumNoteIds) === 2 &&
          this.typeCount(note, kickDrumNoteIds) > 0
        ) {
          this.addNoteIssue('difficultyForbiddenNote', note.time);
        }
      }
  ...
  private setSustainProperties() {
    ...
    for (let i = 0; i < this.notes.length; i++) {
      ...
      if (
        futureSustainGaps.find(
          r => note.time >= r.startTime && note.time < r.endTime,
        )
      ) {
      ...
      if (note.length > 0) {
        ...
        if (note.length < MIN_SUSTAIN_MS) {
          ...
          for (
            let j = i + 1;
            j < this.notes.length &&
            this.notes[j].time <= note.time + note.length;
            j++
          ) {
            if (this.notes[j].type === EventType.open) {
              const matchingEvents = this.trackEvents.filter(
                te => te.time === this.notes[j].time,
              );
  ...
  private setNpsProperties() {
    ...
    this.notesData.maxNps.push({
      ...
      notes: this.notes.filter(
        n => n.time > noteRegionMin && n.time <= noteRegionMax,
      ),
    });
  }
}
