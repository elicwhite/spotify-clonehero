...
export async function getSpotifyAccessToken(userId: string): Promise<ProtectedAccessToken | null> {
  ...
  await supabase
    .from('spotify_tokens')
    .update({ access_token: newAccessToken, expires_at: newExpiresAt.toISOString() })
    .eq('user_id', userId)
