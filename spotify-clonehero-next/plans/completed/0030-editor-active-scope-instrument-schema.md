...
- **`state.chart` and `state.track` retained as scope-aware caches.** Originally the plan called for dropping both slices in this phase. In practice they remained because too many call sites read them synchronously inside event handlers; converting them to selector calls would have widened the diff substantially. They are now driven from `state.activeScope` via `findTrackInParsedChart` inside `EXECUTE_COMMAND` / `UNDO` / `REDO`, so they correctly reflect the active scope. Final removal is deferred to **phase 8 (plan 0037)** when the reducer moves into `chart-editor-core`.
- **`InteractionManager` and `SceneOverlays` still read `LANE_X_POSITIONS` directly.** Plan §6 (lane positions from schema) was deferred to **phase 9 (plan 0038)**, since no current consumer renders a non-drums highway (`DrumHighwayPreview` synthesizes a fake drum track for lanes-off). Geometry math stays in `lib/preview/highway/` for now; phase 9 will wire `LaneDefinition` → renderer once `EditorProjection` lands.
- **`LaneDefinition.worldXOffset` was dropped from the schema.** Including it would have created a `lib/chart-edit` → `lib/preview/highway` coupling, which violates the layering. Renderer keeps `LANE_X_POSITIONS` and `calculateNoteXOffset`; schema only carries display data + scan-chart `NoteType`. Phase 9 will reorganize geometry alongside projections.
- **`EntityKindHandler` shape changed minimally.** Added an optional `EntityContext = { trackKey?: TrackKey; partName?: string }` parameter to `listIds` / `locate` / `move`. Default fallback resolves to drums-expert / `'vocals'` so backward-compat is preserved during the transition. Phase 8 (plan 0037) replaces this with `EntityAdapter` + structured `EntityRef`.
- **Drum-specific commands take optional `trackKey?: TrackKey`.** `AddNoteCommand`, `DeleteNotesCommand`, `ToggleFlagCommand` carry the target track at construction time rather than reading it from the reducer at execute time. This is correct for undo/redo, where scope may have changed between original execute and a later replay. `MoveEntitiesCommand` accepts the broader `ctx?: EntityContext`.
...
- **`PRO_DRUMS_MODIFIERS` consolidation.** `useEditCommands.ts` reads `doc.parsedChart.iniChartModifiers ?? defaultIniChartModifiers` (scan-chart already exports `defaultIniChartModifiers`). Pages that were instantiating empty drum charts (`drum-edit`, `chart-review`, `drum-transcription/EditorApp`) now build their modifiers as `{ ...defaultIniChartModifiers, pro_drums: true }`.
...
- **Entity tests retained `LANE_ORDER` inline.** Phase 8 will move lane ordering into the schema; for now the entity dispatch keeps a private const so the test surface doesn't churn twice.
