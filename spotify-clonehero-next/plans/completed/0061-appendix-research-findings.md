...
and which files are, by design, a running ledger of _rejected_ ideas. This
...
1. **`autoresearch-pipeline/program.md` contains THREE appended blocks for
   ...
   - `# INTERACTIVE PHASE — CORRECTED 3-OP MATRIX` (~line 2157) — its own
     header says why it exists: _"measure each interaction under the
     product's THREE note-handling ops."_ This block's RE-PREDICT row
...
same corpus, holding the _new_ tempo map fixed and varying only what happens
...
| **RE-PREDICT**                   | Note is re-derived from scratch: the corrected tempo map is re-fit to the song's raw drum-onset detections (not to the note's _current_ position), then the onset is re-quantized through the corrected lattice. | Structural corrections that change what the lattice _means_ — half/double-time flip, meter change — see 0061 §3a class (b) primary path.                                                                                                                             |
...
- **A small, local error** (a marker's exact ms position is off by a few
  tens of ms) — the _lattice's meaning_ (which beat is which, how many
...
- **A structural error** (the whole song was transcribed at half or double
  the true tempo, or the meter is wrong) — every note's _tick_ was assigned
  ...
  but land on garbage subdivisions relative to the _new_, correct lattice —
  ...
  `msTime` to the new lattice) does fix the label but was measured to _also_
  ...
  **RE-PREDICT** fixes this by throwing away the stale note positions
  ...
  encoded the wrong lattice, so re-quantizing _them_ (not the notes) avoids
...
| ---------------------------------------------------------------------------------------- | -------------------------------------------------------------- | --------------------------------- | ------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Downbeat tap → bar relabel                                                               | any (op-invariant)                                             | ≈ 0 (+0.0002)                     | **+1.17pp** corpus-wide, **+4.19pp** on the worst-scoring 191-song cohort | Op-invariant because bar relabel is not a note op at all (see below) — same result no matter which of the other three ops a _different_ edit on the same song would use.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| Structural correction                                                                    | RE-PREDICT, **guarded** (batch/automated use, no live preview) | **−0.004** (better than baseline) | +1.27pp                                                                   | Certification COMPLETED 2026-07-19 with a blocking verdict: the note-fit guard alone is **proven insufficient** (a wrong octave bit densifies the grid, which trivially _improves_ note-fit while destroying groove — 78% of abuse cases slip through). Containment requires an additional groove-regularity gate that is **ground-truth-referenced and cannot run at inference**. The batch path is therefore **blocked on a deployable groove proxy** (measured plausible at ~93% best-split via decoded-onset density/phase features, but not built). See 0061 §3a's "Guarded batch/automated path — BLOCKED" for the full verdict. The numbers in this row remain valid for the _correct-bit_ case and describe what a future proxy-guarded path would deliver. |
...
A downbeat/bar-1 correction — the user identifies that a _different_ beat
...
| 0061 §3a (guarded batch path)     | The guarded RE-PREDICT row (−0.004 / +1.27pp) and its BLOCKED-on-deployable-groove-proxy verdict (certification completed 2026-07-19).                                                                                                                                                                                                                               |
